...
static void heapify_down(min_heap *source) {
	...
	// your code here
	int *p = source->values;
	for (int i = source->count / 2 - 1; i >= 0; i--) {
		int j = i;
		while (2 * j + 1 < source->count) {
			int left_child = 2 * j + 1;
			int right_child = 2 * j + 2;
			int largest = j;
			if (right_child < source->count && p[right_child] > p[largest]) {
				largest = right_child;
			}
			if (left_child < source->count && p[left_child] > p[largest]) {
				largest = left_child;
			}
			if (largest != j) {
				heap_swap((p + j), (p + largest));
				j = largest;
			} else {
				break;
			}
...
int min_heap_valid(const min_heap *source) {
	...
	for (int i = 0; i < source->count; i++) {
		int left_child_index = 2 * i + 1, right_child_index = 2 * i + 2;
		if ((left_child_index < source->count
				&& source->values[left_child_index] < source->values[i])
				|| (right_child_index < source->count
						&& source->values[right_child_index] < source->values[i])) {
...
void heap_sort(int *values, int count) {
	...
	for (int i = count / 2 - 1; i >= 0; i--) {
		heapify_down(values, i, count);
	}

	// Extract elements from heap in descending order
	for (int i = count - 1; i > 0; i--) {
		heap_swap(&values[0], &values[i]);
		heapify_down(values, i, count);
	}
}
