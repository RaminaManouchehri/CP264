...
void test_bst(void) {
	...
	for (int i = 0; i < count; i++) {
		bst_insert(target, items[i]);
	}
	printf("Are the two trees equal: %s\n",
			BOOL_TO_STR(bst_equals(source, target)));

	int zero = 0, one = 0, two = 0;
	bst_node_counts(source, &zero, &one, &two);
	printf("Node Counts: Zero Child: %d, One Child: %d, Two Children: %d\n",
			zero, one, two);
